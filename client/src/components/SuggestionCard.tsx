...
const SuggestionCard = ({ suggestion, showAppraisal = false }: SuggestionCardProps) => {
  const { isAuthenticated } = useAuth();
  const queryClient = useQueryClient();
  const [liked, setLiked] = useState(false);
  const [appraisalCount, setAppraisalCount] = useState(suggestion.appraisalCount || 0);

  // Fetch initial liked status and count
  const { data: appraisalStatus } = useQuery({
    queryKey: ['appraisalStatus', suggestion.id, 'suggestion'],
    queryFn: () => appraisalService.getStatus(suggestion.id, 'suggestion'),
    enabled: showAppraisal,
  });

  const toggleAppraisalMutation = useMutation({
    mutationFn: () => appraisalService.toggle(suggestion.id, 'suggestion'),
    onMutate: async () => {
      // Cancel any outgoing refetches
      await queryClient.cancelQueries({ queryKey: ['appraisalStatus', suggestion.id, 'suggestion'] });
      
      // Snapshot the previous value
      const previousStatus = queryClient.getQueryData(['appraisalStatus', suggestion.id, 'suggestion']);
    ...
    onSuccess: (data) => {
      ...
      queryClient.setQueryData(['appraisalStatus', suggestion.id, 'suggestion'], {
        liked: data.liked,
        count: data.count
      });
    ...
    onError: (err, variables, context) => {
      ...
      if (context?.previousStatus) {
        queryClient.setQueryData(['appraisalStatus', suggestion.id, 'suggestion'], context.previousStatus);
  ...
  useEffect(() => {
    ...
    } else if (!appraisalStatus && suggestion.appraisalCount !== undefined && !isPending) {
      // Fallback to count from suggestion data if status not yet loaded
      setAppraisalCount(suggestion.appraisalCount);
    }
  }, [appraisalStatus, suggestion.appraisalCount, isPending]);
    ...
    >
      ...
      {suggestion.images && suggestion.images.length > 0 && (() => {
        ...
        const imageUrl = suggestion.images[0].startsWith('http') 
          ? suggestion.images[0] 
          : `${serverBaseUrl}${suggestion.images[0].startsWith('/') ? suggestion.images[0] : '/' + suggestion.images[0]}`;
        ...
        return (
          <img
            ...
            onError={(e) => {
              console.error('Image failed to load:', imageUrl, 'Original:', suggestion.images[0]);
      ...
      <div className={`p-6 ${suggestion.isTrending ? 'pt-8' : ''}`}>
        <div className="mb-2 flex items-center justify-between">
          <div className="flex items-center space-x-2">
            {suggestion.case_id && (
              <div>
                <span className="text-xs text-gray-500 dark:text-gray-400 font-medium">Case ID: </span>
                <span className="text-xs text-blue-600 dark:text-blue-400 font-bold">{suggestion.case_id}</span>
              </div>
            )}
            {/* Public/Private Indicator */}
            {(suggestion.isPublic || (suggestion as any).is_public) ? (
        ...
        <div className="flex items-start justify-between mb-2">
          <h3 className="text-xl font-semibold text-gray-900 dark:text-white flex-1">{suggestion.title}</h3>
          <span className="ml-2 px-2 py-1 rounded-full text-xs font-medium bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-200">
            💡 Suggestion
          </span>
        </div>
        {suggestion.description && (
          <p className="text-gray-600 dark:text-gray-400 mb-4 line-clamp-2">{suggestion.description}</p>
        )}
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            <span className={`px-2 py-1 rounded-full text-xs font-medium ${statusColors[suggestion.status] || statusColors.under_review}`}>
